...
כללי ברזל:
1. השתמש בכל המספרים שסופקו — ציין ערכים מספריים ספציפיים (לא "גבוה" אלא "23.4%"). אל תמציא מספרים שלא מופיעים בנתונים.
2. כל ניתוח חייב להכיל לפחות 2 מספרים ספציפיים מהנתונים שסופקו.
3. כתוב בעברית — חד, ישיר, אנליטי. כל משפט חייב לנוע כסף. אין מקום לקלישאות.
4. לכל DCF — חשב ומציין: קצב צמיחה % הנחת WACC % ושווי הוגן מחושב $X למניה.
5. לכל תרחיש (שורי/בסיס/דובי) — ציין מחיר יעד ספציפי ומכפיל P/E צפוי.
6. בכל סיכון — ציין מה יהיה ה-trigger האמיתי וב-% כמה ייפגע הערך.
